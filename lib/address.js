...
const { hexEncode, base32Encode, base32Decode, encodeAccountNumber } = require('./encoding');
...
function isValid (address) {
  return (
    ...
    address.substr(56, 8) == base32Encode(blake2b.blake2b(base32Decode('1111' + address.substr(4, 52)).slice(3), null, 5).reverse())
  );
}
...
function generateAddressFromSecretKey(secret) {
  ...
  return { secret: hexEncode(secret), address: address };
}
